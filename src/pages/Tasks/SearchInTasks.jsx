...
const SearchInCategories = (props) => {
	...
	const [productsTasks_id, setProductsTasks_id] = useState("");
	const [serviceTasks_id, setServiceTasks_id] = useState("");
	...
	useEffect(() => {
		if (productsTasks_id !== "") {
			const fetchData = async () => {
				try {
					const response = await filterCategories(productsTasks_id);
	...
	}, [productsTasks_id, filterCategories, tabSelected]);
	...
	useEffect(() => {
		if (serviceTasks_id !== "") {
			const fetchData = async () => {
				try {
					const response = await filterCategories(serviceTasks_id);
	...
	}, [serviceTasks_id, filterCategories, tabSelected]);
			...
			<div className=' d-flex flex-md-row flex-column  justify-content-start align-items-center gap-3 mb-2 gap-2'>
				<div className='select-input w-100 w-md-49'>
					<FormControl sx={{ width: "100%" }}>
						<Select
							name='serviceTasks_id'
							value={serviceTasks_id}
							onChange={(e) => {
								setServiceTasks_id(e.target.value);
							}}
							sx={selectStyle}
							displayEmpty
							...
							<MenuItem
								className='souq_storge_Tasks_filter_items'
								sx={{
				...
				<div className='select-input  w-100 w-md-49'>
					<FormControl sx={{ width: "100%" }}>
						<Select
							name='productsTasks_id'
							value={productsTasks_id}
							onChange={(e) => {
								setProductsTasks_id(e.target.value);
							}}
							sx={selectStyle}
							displayEmpty
							IconComponent={IoIosArrowDown}
